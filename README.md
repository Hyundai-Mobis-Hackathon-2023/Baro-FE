...
# 🔥배 포 링 크🔥
https://f741bfb0.baro.pages.dev
(데스크톱 환경에서만 접속해주세요)

## 핵심 포인트
1. 미래 사회 완전 자율 주행 시대에서, 기존 이동 수단의 용도를 초월한 `Active Mobility(이동형 공간)`으로 재해석하다.
2. 사용자의 활용목적에 따라 필요한 가구를 `직접 선택`하며, 미리 볼 수 있는 차량 내부의 `3D 배치`로 쉽고 빠르게 나만의 차량을 제작할 수 있다.
3. 자유롭게 배치한 가구에 알맞은 인포테인먼트가 제공되고, 사용자는 `추가적인 테마별 위젯`을 선택하여 PBV에서의 다양한 활용을 누릴 수 있다.

## 프로젝트 기획과 배경 및 목적
현대 모비스에서 얼마 전 선보인 모바일 리빙 서비스와 같이, 미래의 완전 자율 주행 시대에서 차량은 `기존 이동 수단의 목적을 초월`하여 내부에서도 다양한 형태의 생활을 할 수 있는 Active Mobility(이동형 공간)으로 인식이 변화할 것이다.
...
### 3. Devops & 무중단 배포 아키텍처 구현 및 성능 최적화 
