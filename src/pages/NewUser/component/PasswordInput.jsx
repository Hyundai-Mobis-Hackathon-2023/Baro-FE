...
const PasswordInput = ({ currentPage, setCurrentPage, password, setPassword }) => {
  ...
  const moveToNext = () => {
    ...
    console.log(parseInt(localStorage.getItem('userIdx')));
    axios
      .post(`${process.env.REACT_APP_API}/user/signup/makePassword`, {
  ...
  return (
    <StyledMotion initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
      ...
      <Margin height='6' />
      <Typography alertText color={isRightPW || password === '' ? 'gray' : 'alertRed'}>
      ...
      />
      <Margin height='212' />
      <Button bgColor={password === '' ? 'gray' : 'mainRed'} onClick={moveToNext}>
        다음
      </Button>
    </StyledMotion>
  );
